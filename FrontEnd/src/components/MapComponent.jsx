...
const CATEGORY_MAP = {
  ...
  // add other machine values mapping if needed
};
...
export default function MapComponent({ onResults = () => {}, onReady = () => {} }) {
  ...
  // POST score request to backend
  const fetchScore = async ({ address, lat, lon, radius, category, type }) => {
    try {
      const apiBase = import.meta.env.VITE_API_URL || "http://localhost:8080";
      ...
      const payload = await res.json();
      return payload?.data ?? null; // expected { result, score }
    } catch (err) {
        ...
        L.popup()
          ...
          .setContent(
            `<strong>${poi.name || poi.category}</strong><div>${
              poi.tags ? Object.entries(poi.tags).slice(0, 3).map(([k, v]) => `${k}=${v}`).join(", ") : ""
            }</div>`
          )
  ...
  function selectLocation(lat, lon, label = "") {
    ...
    window.dispatchEvent(new CustomEvent("poi-selected", { detail: { lat: +lat, lon: +lon, label: label || "" } }));
  }
    ...
    try {
      ...
      if (overpass && Array.isArray(overpass.elements)) {
        for (const el of overpass.elements) {
          ...
          if (tags.aeroway && ["aerodrome", "helipad"].includes(tags.aeroway)) aerodromes.push(item);
      ...
      poisList.sort((a, b) => {
        const ca = categoryScore(a, catKey), cb = categoryScore(b, catKey);
      ...
      topPois.forEach((p) => {
        ...
        m.bindPopup(`<div class="popup-content"><strong>${p.name || p.category}</strong><div class="popup-meta">${p.distance_m} m • ${p.category}</div></div>`);
        group.addLayer(m);
      });
      ...
      // attach user data returned by backend (best-effort)
      try {
        const userDoc = await postUserData({
          lat: agg.selection.lat,
          lon: agg.selection.lon,
        ...
        // already handled in postUserData; ignore
      }

      // fetch suitability score (best-effort)
            // propagate aggregated result to parent
            // fetch suitability score (best-effort) — POST payload expected by backend
      try {
        ...
        if (scoreData) {
          agg.scoreResult = scoreData; // expected shape: { result, score }
        }
      } catch (err) {
        // fetchScore logs errors; continue
      }
      ...
      try {
        ...
        console.warn("onResults handler error", err);
      }
